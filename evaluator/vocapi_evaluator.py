...
class VOCAPIEvaluator():
    ...
    Three things would be saved every evaluation process
    1. annots.pkl:  'VOCdevkit/VOC2007/Annotations/annots.pkl'
    The recs annotation results of the corresponding datasets, only need to save once

    2. prediction results:   # VOCdevkit/VOC2007/results/experiment_name/det_test_aeroplane.txt
    filename = 'det_' + self.set_type + '_%s.txt' % (cls)
    filedir = os.path.join(self.devkit_path, 'results', 'experiment_name')

    ## everytime, you must change the experiment_name to save every experiment results, or it would be occupied
    saving format: ('{:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.format(index[1], dets[k, -1],dets[k, 0] + 1, dets[k, 1] + 1, dets[k, 2] + 1, dets[k, 3] + 1) )

    3. metrics result:    # ./voc_eval/test/experiment_name/aeroplane_pr.pkl
    The quantitative results of object detection
    dir = self.output_dir, cls + '_pr.pkl'
    saving format: {'rec': rec, 'prec': prec, 'ap': ap}
    ...
    def __init__(self, 
        ...
        # path
        self.experiment_name = save_name
        ...
        # self.output_dir = self.get_output_dir(self.devkit_path, self.set_type)
        self.output_dir = self.get_output_dir(os.path.join('voc_eval/',self.set_type), self.experiment_name)
    ...
    def evaluate(self, net):
        ...
        if self.distributed:
            ...
            sampler = range(num_images)
        # for i in tqdm(range(num_images)):

        for i in tqdm(sampler):

            if self.data_type == 'Exposure_Event':
                im, event, _ = self.dataset.pull_img_event(i)
                h, w, _ = im.shape
                ...
                assert event.shape[0] == 2
                event = event.unsqueeze(0).to(self.device)
        ...
        print('Final Mean AP_0.5', self.ap50)
        print('Final Mean AP_0.75', self.ap75)
        print('Final Mean AP_0.95', self.ap95)
    ...
    def get_voc_results_file_template(self, cls):
        ...
        filedir = os.path.join(self.devkit_path, 'results', self.experiment_name)          ## add experiment root
        if not os.path.exists(filedir):
            os.makedirs(filedir)
    ...
    def do_python_eval(self, use_07=True):
        ...
        overthres = [0.5, 0.75, 0.95]
        for overthreshold in overthres:
            aps = []
            for i, cls in enumerate(self.labelmap):
                ...
                aps += [ap]
                print('mAP_{} for {} = {:.4f}'.format(overthreshold, cls, ap))
                with open(os.path.join(self.output_dir, cls + '_pr.pkl'), 'wb') as f:
                    pickle.dump({'rec': rec, 'prec': prec, 'ap': ap}, f)
            
            if overthreshold == 0.5:
                self.ap50 = np.mean(aps)
            elif overthreshold == 0.75:
                self.ap75 = np.mean(aps)
            elif overthreshold == 0.95:
                self.ap95 = np.mean(aps)
    ...
    def voc_eval(self, detpath, classname, cachedir, ovthresh=0.5, use_07_metric=True):
        ...
        for imagename in imagenames:
            ...
            difficult = np.array([x['difficult'] for x in R]).astype(bool)    ## np.bool --> bool  for updated numpy version >= 1.20.0
            det = [False] * len(R)
