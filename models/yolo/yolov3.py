...
class YOLOv3(nn.Module):
    def __init__(self, 
        ...
        self.backbone, feature_channels, strides = build_backbone(model_name=cfg["backbone"], 
                                                                  pretrained=trainable, in_channel=in_channel)
        self.stride = strides
    ...
    def forward(self, x, targets=None):
        if not self.trainable:
            ...
            # p5/32
            p5 = self.head_convblock_0(c5)
            # print(f'.head_convblock_0 p5 shape:{p5.shape}')
            p5_up = self.head_upsample_0(self.head_conv_0(p5))
            # print(f'.head_upsample_0 p5_up shape:{p5_up.shape}')
            p5 = self.head_conv_1(p5)
            # print(f'.head_conv_1 p5 shape:{p5.shape}')

            # p4/16
            p4 = self.head_convblock_1(torch.cat([c4, p5_up], dim=1))
            # print(f'.head_convblock_1 p4 shape:{p4.shape}')
            p4_up = self.head_upsample_1(self.head_conv_2(p4))
            # print(f'.head_upsample_1 p4_up shape:{p4_up.shape}')
            p4 = self.head_conv_3(p4)
            # print(f'.head_conv_3 p4 shape:{p4.shape}')

            # P3/8
            p3 = self.head_convblock_2(torch.cat([c3, p4_up], dim=1))
            # print(f'.head_convblock_2 p3 shape:{p3.shape}')
            p3 = self.head_conv_4(p3)
            # print(f'.head_conv_4 p3 shape:{p3.shape}')
            ...
            # print(f'pred_s shape:{pred_s.shape}, pred_m shape:{pred_m.shape}, pred_l shape:{pred_l.shape}')
            ...
            # print(f'obj_pred shape:{obj_pred.shape}, cls_pred shape:{cls_pred.shape}, box_pred shape:{box_pred.shape}')
            ...
            # giou: [B, HW,]
            giou_pred, _ = box_ops.giou_score(x1y1x2y2_pred, x1y1x2y2_gt, batch_size=B)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cuda', action='store_true', default=False,
                        help='Use cuda')
    parser.add_argument('--save_name', default='yolov3_RGB_4gpu_16bs_50epoch_0.001lr', type=str)
