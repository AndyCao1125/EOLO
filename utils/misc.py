...
def detection_collate_RGBEvent(batch):
    """Custom collate fn for dealing with batches of images that have a different
    number of associated object annotations (bounding boxes).
    ...
    Return:
        A tuple containing:
            1) (tensor) batch of images stacked on their 0 dim and (tensor) batch of events stacked on their 0 dim
            2) (list of tensors) annotations for a given image are stacked on
                                 0 dim
    """
    targets = []
    ...
    for sample in batch:
        imgs.append(sample[0])
        events.append(sample[1])
        targets.append(torch.FloatTensor(sample[2]))
    return torch.stack(imgs, 0), torch.stack(events, 0), targets
