...
class ResizeEvent(object):
    ...
    def __call__(self, image):
        zeros_channel = np.zeros((image.shape[0], image.shape[1], 1))
        image = np.concatenate((image, zeros_channel), axis=2)
        h0, w0, _ = image.shape
        ...
        image_ = cv2.resize(image, (self.size, self.size)).astype(np.float32)
        ## remove the last zero channel:
        image_ = image_[:, :, 0:2]
        return image_

class ResizeEventFrame(object):
    def __init__(self, size=640):
        self.size = size
...
class EventTransforms(object):
    def __init__(self, size=640, norm=False):
        self.size = size
        if norm:
            self.augment = ComposeEvent([
                ResizeEvent(self.size),
                NormalizeEvent(),
                ToEventTensor()
            ])
    ...
    def __call__(self, event):
        return self.augment(event)


class EventFrameTransforms(object):
    def __init__(self, size=640, norm=False):
        self.size = size
        if norm:
            self.augment = ComposeEvent([
                ResizeEventFrame(self.size),
                NormalizeEvent(),
                ToEventTensor()
            ])
...
if __name__ == '__main__':
    trans = EventFrameTransforms()
    event_frame = cv2.imread('/home/dataset/VOC_dataset/VOCdevkit/VOC2007/EventFrameImages/Underexposure_0.2/000001.jpg')
    out = trans(event_frame)
    print(out.shape)