...
class VOCDetection(data.Dataset):
    ...
    def __init__(self, 
        ...
        if data_type == 'Exposure_Event' and exposure_factor is not None:
            self._imgpath = osp.join('%s', 'ExposureImages', str(exposure_factor), '%s.jpg')
            self._eventpath = osp.join('%s', 'Event', str(exposure_factor), '%s.npy')
            print(f'----->Loading {data_type} images ...')

        else:
            raise ValueError('Wrong data_type or lack exposure_factor')

        ## Loading image sets [train_new, val_new, test_new]
        self.ids = list()
        ...
        print(f'------------------Loading {image_sets} dataset with size {len(self.ids)}------------------')
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == 'Exposure_Event':
            im, event, gt, h, w, scale, offset = self.pull_item(index)
            return im, event, gt
    ...
    def load_event_targets(self, img_id):
        # load an image
        event = np.load(self._eventpath % img_id)
        height, width, channels = event.shape
        ...
        return event, target, height, width
    
    def load_img_event_targets(self, img_id):
        # load an image
        img = cv2.imread(self._imgpath % img_id)
        event = np.load(self._eventpath % img_id)
        height, width, channels = img.shape
        ...
        return img, event, target, height, width

    def load_img_eventframe_targets(self, img_id):
        # load an image
        img = cv2.imread(self._imgpath % img_id)
        eventframe = cv2.imread(self._eventpath % img_id)
        height, width, channels = img.shape
        ...
        return img, eventframe, target, height, width
    ...
    def pull_item(self, index):
        ...
        # load an image and target
            ...
            if self.data_type == 'Exposure_Event':
                img, event, target, height, width = self.load_img_event_targets(img_id)
                event = self.event_transform(event)   ## resize event using the normal cv2.resize (linear interpolation by default)
        ...
        if self.data_type == 'Exposure_Event':
            return img, event, target, height, width, scale, offset
    ...
    def pull_img_event(self, index):
        ...
        return cv2.imread(self._imgpath % img_id, cv2.IMREAD_COLOR), event, img_id
    
    def pull_img_eventframe(self, index):
        '''Returns the original image object at index in PIL form
        ...
        '''
        img_id = self.ids[index]
        eventframe = cv2.imread(self._eventpath % img_id, cv2.IMREAD_COLOR)

        return cv2.imread(self._imgpath % img_id, cv2.IMREAD_COLOR), eventframe, img_id
